...
$(document).ready(function() {
  ...
  getWeather("40.0150","-105.2705");
  $("#newCoord").submit(function(event) {
    event.preventDefault();
    getWeather(document.getElementById("latitude").value,document.getElementById("longitude").value);
  });
  
  function getWeather(lat,lon) {
    var url ='https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query='+lat+','+lon+'&forecast_days=6'; //Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5
    console.log(url);
    $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
      ...
      $("#heading").html("Today's Weather Forcast - " + data.location.name + ", " + data.location.region);
      
      var today_temperature = getFarenheitTemp(parseInt(data.current.temperature));
      var outer_height = $("#thermometer_outer").height();
      $("#temp_today").html(today_temperature + "\u00B0F");
      $("#thermometer_inner").height((today_temperature/100)*outer_height);
      if (today_temperature >= 85) {
        $("#thermometer_inner").css("background-color","red");
      }else if (today_temperature >= 65) {
        $("#thermometer_inner").css("background-color","gray");
      }else {
        $("#thermometer_inner").css("background-color","blue");
      }
      $("#local_time").html(current_time);
      $("#precip_today").html(data.current.precip + "mm");
      $("#humidity_today").html(data.current.humidity + "%");
      $("#wind_today").html(data.current.wind_speed + "km/h " + data.current.wind_dir);
      $("#summary_today").html(data.current.weather_descriptions);
      $("#latitude").attr("value",lat);
      $("#longitude").attr("value",lon);
      //5 day forcast
      $("#5_day_forecast").html(newCard(1)+newCard(2)+newCard(3)+newCard(4)+newCard(5));
      ...
      */
      ...
      function newCard(i) {
        var thisDay = getKey(i);
        var weekDay = getDayofWeek(thisDay.date.substring(8,10),thisDay.date.substring(5,7),thisDay.date.substring(0,4))
        var high = getFarenheitTemp(parseInt(thisDay.maxtemp));
        var low = getFarenheitTemp(parseInt(thisDay.mintemp));
        var sunrise = thisDay.astro.sunrise;
        var sunset = thisDay.astro.sunset;
        return '<div style="width: 20%;"> \
          <div class="card"> \
            <div class="card-body"> \
              <h5 class="card-title">'+weekDay+'</h5> \
              <p class="card-text">High: '+high+'<br> \
                Low: '+low+' <br> \
                Sunrise: '+sunrise+'<br> \
                Sunset: '+sunset+' \
              </p> \
