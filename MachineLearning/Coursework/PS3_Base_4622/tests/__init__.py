...
def test_ridge_coef(Ridge, normalize):
    ...
    tester.add_test("1.2.a", test_coef)
    tester.run()
...
def test_lasso_coef(Lasso, normalize):
    ...
    tester.add_test("1.4.a", test_coef)
    tester.run()
...
def test_elastic_coef(Elastic, normalize):
    tester = Tester()
    features = np.array([[1.55143777, 0.2644804, 0.0995576],
                         [0.22541014, 1.6967911, -0.45701382],
                         [0.12528546, -1.44263567, 0.7017054],
    ...
    coef_ = np.array([53.27862827, 10.94209836, 33.96771899])
    if normalize:
        coef_ = np.array([48.11601119, 14.15394196, 38.74380055])

    if normalize is None:
        reg = Elastic(alpha=2, beta=0.5)
    else:
        reg = Elastic(alpha=2, beta=0.5, normalize=normalize)
    reg.fit(features, labels)

    ins = "\n X:" + str(features) + ",\ntargets:" + str(labels)
    topic = "Testing Elastic's fit, normalize: {}, alpha:{}, beta:{}".format(normalize, 2.0, 0.5)
    ...
    tester.add_test("1.7.a", test_coef)
    tester.run()


def test_elastic_intercept(Elastic, normalize):
    tester = Tester()
    features = np.array([[1.55143777, 0.2644804, 0.0995576],
                         [0.22541014, 1.6967911, -0.45701382],
                         [0.12528546, -1.44263567, 0.7017054],
    ...
    intercept = -7.796083758223746
    if normalize:
        intercept = -7.502412860000006
    if normalize is None:
        reg = Elastic(alpha=2, beta=0.5)
    else:
        reg = Elastic(alpha=2, beta=0.5, normalize=normalize)
    ...
    ins = "\n X:" + str(features) + ",\ntargets:" + str(labels)
    topic = "Testing Lasso's intercept, normalize: {}, alpha:{}, beta:{}".format(normalize, 2.0, 0.5)
